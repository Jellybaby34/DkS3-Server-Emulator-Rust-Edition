...
impl RsaManager {
    pub fn new(config: Config) -> RsaManager {
        let rsa_private_key_str = config.get_rsa_private_key();
        let rsa_priv_key_inst = Rsa::private_key_from_pem(rsa_private_key_str.as_bytes()).unwrap();

        RsaManager { rsa_priv_key_inst }
    }
    ...
    pub fn rsa_encrypt(&self, from: &[u8], to: &mut [u8]) -> usize {
        ...
        let length = self
            .rsa_priv_key_inst
            .private_encrypt(from, to, Padding::from_raw(5))
            .unwrap();
        length
    }
    ...
    pub fn rsa_decrypt(&self, from: &[u8], to: &mut [u8]) -> usize {
        let length = self
            .rsa_priv_key_inst
            .private_decrypt(from, to, Padding::PKCS1_OAEP)
            .unwrap();
        length
    }
}
